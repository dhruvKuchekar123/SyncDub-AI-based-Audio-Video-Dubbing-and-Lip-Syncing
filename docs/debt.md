...
| 6 | Unpinned dependencies; XTTS lives in a second, undocumented conda env | Fresh clones behave differently; onboarding is folklore | First non-founder engineer's first day (hire #1) | Partially repaid — `requirements.txt` corrected & env documented in `.env.example` (Stage 0); pinning still open |
