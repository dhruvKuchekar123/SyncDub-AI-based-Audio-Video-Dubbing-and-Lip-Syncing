...
2. **Rate adjustment within tolerance.** Time-stretch synthesized audio toward target duration, capped (±10–12% is generally imperceptible; the rate-deviation metric enforces the cap). Edge-TTS accepts a rate parameter natively; XTTS output can be stretched post-hoc with standard DSP. Cheap, safe inside the cap, ugly beyond it.
